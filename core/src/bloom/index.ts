 ...
 */
export function xxh64String(value: string, seed: bigint = 0n): bigint {
 ...
 */
export function xxh64Number(value: number, seed: bigint = 0n): bigint {
 ...
 */
export function xxh64BigInt(value: bigint, seed: bigint = 0n): bigint {
  const buffer = new ArrayBuffer(8);
  const view = new DataView(buffer);
 ...
 */
export function calculateOptimalBlocks(
  expectedItems: number,
  fpp: number = DEFAULT_FPP,
 ...
 */
export function estimateFalsePositiveRate(numBlocks: number, itemCount: number): number {
...
export class BloomFilter {
  ...
  constructor(options: BloomFilterOptions = {}) {
    const {
      expectedItems = 10000,
      falsePositiveRate = DEFAULT_FPP,
      maxBytes = DEFAULT_MAX_BYTES,
      numBlocks: providedBlocks,
    } = options;
   ...
   */
  private computeBlockMask(hash: bigint): { blockIndex: number; masks: Uint32Array } {
   ...
   */
  private hashValue(value: string | number | bigint | Uint8Array): bigint {
  ...
  merge(other: BloomFilter): void {
    if (other.numBlocks !== this.numBlocks) {
      throw new Error(
        `Cannot merge bloom filters with different block counts: ${this.numBlocks} vs ${other.numBlocks}`
      );
    }
...
/**
 * Column filter entry being built
 */
...
export class BloomFilterWriter {
  ...
  private filters: Map<number, ColumnFilterEntry> = new Map();
  ...
  addValues(
    fieldId: number,
    columnName: string,
    values: Iterable<string | number | bigint | Uint8Array>
  ): void {
    const filter = this.getOrCreateFilter(fieldId, columnName);
  ...
  finalize(): { data: Uint8Array; metadata: BloomFilterMetadata[] } {
    ...
    const serializedFilters: { entry: ColumnFilterEntry; nameBytes: Uint8Array; filterData: Uint8Array }[] = [];
 ...
 */
export function parseBloomFilterFile(data: Uint8Array): ParsedBloomFilterEntry[] {
  const view = new DataView(data.buffer, data.byteOffset, data.byteLength);
  ...
  const entries: ParsedBloomFilterEntry[] = [];
 ...
 */
export function createBloomFilterMap(entries: ParsedBloomFilterEntry[]): Map<number, BloomFilter> {
  const map = new Map<number, BloomFilter>();
 ...
 */
export function shouldReadFile(
 ...
 */
export function shouldReadFileForAny(
  filter: BloomFilter | undefined,
  values: Iterable<string | number | bigint | Uint8Array>
): boolean {
  if (!filter) {
 ...
 */
export function getBloomFilterPath(dataFilePath: string): string {
