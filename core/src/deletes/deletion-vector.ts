...
export class DeletionVector {
   ...
   */
  private containers: Map<bigint, Set<number>>;
 ...
 */
export function serializeDeletionVector(dv: DeletionVector): Uint8Array {
  ...
  const sortedKeys = Array.from(containers.keys()).sort((a, b) =>
    a < b ? -1 : a > b ? 1 : 0
  );
 ...
 */
export function deserializeDeletionVector(data: Uint8Array): DeletionVector {
  if (data.length < 12) {
  ...
  const dv = new DeletionVector();
  const containers = new Map<bigint, Set<number>>();
 ...
 */
export function createDeletionVectorBlob(
  dv: DeletionVector,
  referencedDataFile: string
): PuffinBlob {
 ...
 */
export function mergeDeletionVectors(dvs: DeletionVector[]): DeletionVector {
