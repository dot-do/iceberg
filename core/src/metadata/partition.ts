...
export function getPartitionData(
  ...
  for (const partitionField of spec.fields) {
    ...
    if (sourceIds !== undefined && sourceIds.length > 0) {
      ...
      throw new Error(
        `Multi-argument transforms (source-ids) are not yet supported for partition data extraction. ` +
    ...
    if (sourceId === undefined) {
      throw new Error(
        `Partition field '${partitionField.name}' must have either 'source-id' or 'source-ids'`
      );
    }
