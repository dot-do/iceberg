...
export type ConflictResolutionStrategy =
  | 'fail' // Fail immediately on conflict (default)
  | 'retry' // Retry with updated metadata
  | 'rebase'; // Rebase changes on top of new metadata
 ...
 */
export function generateVersionedMetadataPath(
  tableLocation: string,
  version: number
): string {
 ...
 */
export function parseMetadataVersion(metadataPath: string): number | null {
 ...
 */
export function getVersionHintPath(tableLocation: string): string {
 ...
 */
export function getMetadataVersion(metadata: TableMetadata): number {
...
export class AtomicCommitter {
   ...
   * Get the version hint file path.
   */
  private getVersionHintPath(): string {
   ...
   */
  async getCurrentMetadataPath(): Promise<string | null> {
    const data = await this.storage.get(this.getVersionHintPath());
    if (!data) return null;
    const text = new TextDecoder().decode(data).trim();
    // Handle both formats: just version number, or full path
   ...
   */
  async loadMetadata(): Promise<TableMetadata | null> {
   ...
   */
  private async writeVersionHint(version: number | string): Promise<void> {
   ...
   */
  private async cleanupFiles(files: string[]): Promise<boolean> {
    ...
    try {
      ...
      if (actualVersion !== expectedVersion) {
        throw new CommitConflictError(
          `Commit conflict: expected version ${expectedVersion}, but current version is ${actualVersion}`,
          expectedVersion,
          actualVersion
        );
      }
      ...
      if (pendingCommit.previousMetadataPath) {
        builder.addMetadataLogEntry(
          pendingCommit.previousMetadataPath,
          pendingCommit.baseMetadata['last-updated-ms']
        );
      }
      ...
      const newMetadataPath = generateVersionedMetadataPath(
        this.tableLocation,
        newVersion
      );
      ...
      if (!written) {
        ...
        throw new CommitConflictError(
          `Commit conflict: metadata file already exists at ${newMetadataPath}`,
          expectedVersion,
          expectedVersion // We don't know the actual version, use expected
        );
      }
      ...
      if (options.cleanupOnFailure !== false && writtenFiles.length > 0) {
        ...
        throw new CommitTransactionError(
   ...
   */
  async commit(
    buildCommit: (metadata: TableMetadata | null) => Promise<PendingCommit>,
    options: CommitOptions = {}
  ): Promise<CommitResult> {
    const {
      maxRetries = COMMIT_MAX_RETRIES,
    ...
    throw new CommitRetryExhaustedError(
  ...
  async commitSnapshot(
    ...
    options: CommitOptions & { schemaId?: number; parentSnapshotId?: number } = {}
  ): Promise<CommitResult> {
  ...
  async cleanupOldMetadata(
    ...
    const {
      retainVersions = METADATA_RETAIN_VERSIONS,
      maxAgeMs = METADATA_MAX_AGE_MS,
      enabled = true,
    } = options;
    ...
    const sortedLog = [...metadataLog].sort(
 ...
 */
export function createAtomicCommitter(
 ...
 */
export async function commitWithCleanup(
  committer: AtomicCommitter,
  buildCommit: (metadata: TableMetadata | null) => Promise<PendingCommit>,
  commitOptions?: CommitOptions,
  cleanupOptions?: MetadataCleanupOptions
): Promise<CommitResult> {
  ...
  committer.cleanupOldMetadata(cleanupOptionsWithoutCallback).catch((error: unknown) => {
    ...
    if (onCleanupFailure) {
      const failureEvent: CleanupFailureEvent = {
        tableLocation: committer.getTableLocation(),
        error: cleanupError,
        timestamp: Date.now(),
        metadataVersion: result.metadataVersion,
