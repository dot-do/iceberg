 ...
 * ```
 */
export class FileSystemCatalog implements IcebergCatalog {
  private readonly catalogName: string;
  private readonly warehouse: string;
  private readonly storage: StorageBackend;
  private readonly defaultProperties: Record<string, string>;
  ...
  private tablePath(identifier: TableIdentifier): string {
    return `${this.namespacePath(identifier.namespace)}/${identifier.name}`;
  }

  private metadataPath(identifier: TableIdentifier): string {
  ...
  async createNamespace(namespace: string[], properties?: NamespaceProperties): Promise<void> {
    ...
    await this.storage.put(
      propsPath,
      new TextEncoder().encode(JSON.stringify(props, null, 2))
    );
  }
  ...
  async getNamespaceProperties(namespace: string[]): Promise<NamespaceProperties> {
    const propsPath = `${this.namespacePath(namespace)}/.namespace-properties.json`;
  ...
  async updateNamespaceProperties(
    ...
    const props = JSON.parse(new TextDecoder().decode(data)) as NamespaceProperties;
  ...
  async createTable(
    ...
    const identifier: TableIdentifier = { namespace, name: request.name };
  ...
  async loadTable(identifier: TableIdentifier): Promise<TableMetadata> {
    const metadataDir = this.metadataPath(identifier);
    ...
    if (versionHintData) {
      ...
      if (metadataFiles.length === 0) {
        throw new Error(
          `Table ${identifier.namespace.join('.')}.${identifier.name} does not exist`
        );
      }
  ...
  async dropTable(identifier: TableIdentifier, purge: boolean = false): Promise<boolean> {
    const tablePath = this.tablePath(identifier);
  ...
  async renameTable(from: TableIdentifier, to: TableIdentifier): Promise<void> {
    ...
    await this.createTable(to.namespace, {
      ...
      partitionSpec: metadata['partition-specs'].find(
  ...
  async commitTable(request: CommitTableRequest): Promise<CommitTableResponse> {
      ...
      switch (req.type) {
        ...
        case 'assert-ref-snapshot-id': {
          ...
          if (currentSnapshotId !== req['snapshot-id']) {
            throw new Error(
              `Ref ${req.ref} snapshot ID mismatch: expected ${req['snapshot-id']}, got ${currentSnapshotId}`
            );
          }
          break;
        }
        case 'assert-current-schema-id':
          if (currentMetadata['current-schema-id'] !== req['current-schema-id']) {
            throw new Error(
              `Current schema ID mismatch: expected ${req['current-schema-id']}, got ${currentMetadata['current-schema-id']}`
            );
          }
          break;
        case 'assert-default-spec-id':
          if (currentMetadata['default-spec-id'] !== req['default-spec-id']) {
            throw new Error(
              `Default spec ID mismatch: expected ${req['default-spec-id']}, got ${currentMetadata['default-spec-id']}`
            );
          }
          break;
      }
 ...
 */
export class MemoryCatalog implements IcebergCatalog {
  private readonly catalogName: string;
  private readonly namespaces: Map<string, NamespaceProperties> = new Map();
  ...
  async dropTable(identifier: TableIdentifier, _purge?: boolean): Promise<boolean> {
    const key = this.tableKey(identifier);
    if (!this.tables.has(key)) {
      return false;
    }
 ...
 */
export function createCatalog(config: CatalogConfig): IcebergCatalog {
  switch (config.type) {
    case 'filesystem':
