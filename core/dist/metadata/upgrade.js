 ...
 * ```
 */
export function upgradeTableToV3(metadata) {
    // Validate source version
    if (metadata['format-version'] === 3) {
        throw new VersionUpgradeError('ALREADY_TARGET_VERSION', 'Table is already at format-version 3. Upgrade is not needed.');
    }
    if (metadata['format-version'] !== 2) {
        throw new VersionUpgradeError('INVALID_SOURCE_VERSION', `Cannot upgrade from format-version ${metadata['format-version']} to v3. ` +
 ...
 */
export function canUpgradeToV3(metadata) {
 ...
 * @throws {VersionUpgradeError} If the source version is not v2 or the metadata is already v3
 */
export function upgradeTableToV3WithOptions(metadata, options = {}) {
