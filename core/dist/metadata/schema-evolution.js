 ...
 * - fixed(L) -> binary
 */
export function isTypePromotionAllowed(fromType, toType) {
    if (fromType === toType)
        return true;
...
export class SchemaEvolutionBuilder {
    ...
    validate() {
        const errors = [];
        let workingSchema = JSON.parse(JSON.stringify(this.currentSchema));
     ...
     */
    build() {
        let newSchema = JSON.parse(JSON.stringify(this.currentSchema));
     ...
     */
    applyOperationToSchema(schema, op, validateOnly) {
        switch (op.type) {
            case 'add-column':
    ...
    applyAddColumn(schema, op, validateOnly) {
        // Check if field already exists
        const existing = schema.fields.find((f) => f.name === op.name);
        if (existing) {
            throw new SchemaEvolutionError(`Column '${op.name}' already exists`, 'FIELD_EXISTS');
        }
        if (!validateOnly) {
    ...
    applyDropColumn(schema, op, validateOnly) {
        const index = schema.fields.findIndex((f) => f.name === op.name);
        if (index === -1) {
            throw new SchemaEvolutionError(`Column '${op.name}' not found`, 'FIELD_NOT_FOUND');
        }
        // Check if this field is in identifier-field-ids
        const field = schema.fields[index];
        const identifierFieldIds = schema['identifier-field-ids'];
        if (identifierFieldIds?.includes(field.id)) {
            throw new SchemaEvolutionError(`Cannot drop identifier column '${op.name}'`, 'IDENTIFIER_FIELD');
        }
        if (!validateOnly) {
    ...
    applyRenameColumn(schema, op, validateOnly) {
        const fieldIndex = schema.fields.findIndex((f) => f.name === op.oldName);
        if (fieldIndex === -1) {
            throw new SchemaEvolutionError(`Column '${op.oldName}' not found`, 'FIELD_NOT_FOUND');
        }
        // Check new name doesn't conflict
        const conflict = schema.fields.find((f) => f.name === op.newName);
        if (conflict) {
            throw new SchemaEvolutionError(`Column '${op.newName}' already exists`, 'FIELD_EXISTS');
        }
        if (!validateOnly) {
    ...
    applyUpdateColumnType(schema, op, validateOnly) {
        ...
        if (!compatibility.compatible) {
            throw new SchemaEvolutionError(`Cannot change type of column '${op.name}': ${compatibility.reason}`, 'INCOMPATIBLE_TYPE');
        }
        if (!validateOnly) {
    ...
    applyMakeColumnOptional(schema, op, validateOnly) {
        const fieldIndex = schema.fields.findIndex((f) => f.name === op.name);
        if (fieldIndex === -1) {
            throw new SchemaEvolutionError(`Column '${op.name}' not found`, 'FIELD_NOT_FOUND');
        }
    ...
    applyMakeColumnRequired(schema, op, validateOnly) {
        const fieldIndex = schema.fields.findIndex((f) => f.name === op.name);
        if (fieldIndex === -1) {
            throw new SchemaEvolutionError(`Column '${op.name}' not found`, 'FIELD_NOT_FOUND');
        }
    ...
    applyUpdateColumnDoc(schema, op, validateOnly) {
        const fieldIndex = schema.fields.findIndex((f) => f.name === op.name);
        if (fieldIndex === -1) {
            throw new SchemaEvolutionError(`Column '${op.name}' not found`, 'FIELD_NOT_FOUND');
        }
    ...
    applyMoveColumn(schema, op, validateOnly) {
        const index = schema.fields.findIndex((f) => f.name === op.name);
        if (index === -1) {
            throw new SchemaEvolutionError(`Column '${op.name}' not found`, 'FIELD_NOT_FOUND');
        }
     ...
     */
    countPreviousAddOperations(currentOp) {
        let count = 0;
        for (const op of this.operations) {
            if (op === currentOp)
                break;
     ...
     */
    resolvePosition(fields, position) {
        switch (position.type) {
            ...
            case 'after': {
                ...
                if (afterIndex === -1) {
                    throw new SchemaEvolutionError(`Reference column '${position.column}' not found for 'after' position`, 'INVALID_POSITION');
                }
                return afterIndex + 1;
            }
            case 'before': {
                ...
                if (beforeIndex === -1) {
                    throw new SchemaEvolutionError(`Reference column '${position.column}' not found for 'before' position`, 'INVALID_POSITION');
                }
                return beforeIndex;
            }
        }
 ...
 */
export function isBackwardCompatible(changes) {
        ...
        switch (change.type) {
            ...
            case 'type-changed':
                ...
                if (typeof change.oldValue === 'string' && typeof change.newValue === 'string') {
                    if (!isTypePromotionAllowed(change.oldValue, change.newValue)) {
                        incompatibleChanges.push(change);
                    }
                }
                else {
                    // Complex type changes need deeper analysis
                    const compat = areTypesCompatible(change.oldValue, change.newValue);
 ...
 * @returns Compatibility result with list of incompatible changes
 */
export function isForwardCompatible(changes) {
    const incompatibleChanges = [];
    for (const change of changes) {
        switch (change.type) {
            case 'added':
 ...
 */
export function isFullyCompatible(changes) {
    ...
    const allIncompatible = [
        ...
        ...forward.incompatibleChanges.filter((c) => !backward.incompatibleChanges.some((bc) => bc.fieldId === c.fieldId && bc.type === c.type)),
    ];
    return {
 ...
 */
export function getSchemaHistory(metadata) {
 ...
 */
export function getSchemaForSnapshot(metadata, snapshotId) {
 ...
 */
export function getSchemaChangesBetween(metadata, fromSchemaId, toSchemaId) {
 ...
 * @returns The field or undefined if not found
 */
export function findFieldById(schema, id) {
 ...
 */
export function getAllFieldIds(schema) {
    const ids = new Set();
 ...
 */
export function evolveSchema(metadata) {
 ...
 */
export function applySchemaEvolution(_metadata, builder) {
 ...
 */
export function evolveNestedStruct(schema, path) {
