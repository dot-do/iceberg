 ...
 * ```
 */
export function createRangePredicate(operator, value) {
    switch (operator) {
        case '$gt':
 ...
 * ```
 */
export function evaluateRangePredicate(predicate, fileLower, fileUpper, _type) {
 ...
 * ```
 */
export function combinePredicatesAnd(predicates) {
    ...
    for (const pred of predicates) {
        ...
        if (pred.lowerInclusive !== undefined) {
            ...
            else if (compareValues(pred.lowerInclusive, lowerInclusive) === 0 &&
        ...
        if (pred.lowerExclusive !== undefined) {
            ...
            else if (lowerInclusive !== undefined &&
        ...
        if (pred.upperInclusive !== undefined) {
            ...
            else if (compareValues(pred.upperInclusive, upperInclusive) === 0 &&
                upperExclusive !== undefined) {
                upperExclusive = undefined;
            }
        }
        if (pred.upperExclusive !== undefined) {
            ...
            else if (upperInclusive !== undefined &&
 ...
 * ```
 */
export function combinePredicatesOr(predicates) {
    if (predicates.length === 0) {
        return [];
    }
    if (predicates.length === 1) {
    ...
    const ranges = predicates.map((pred) => {
        // Handle points
    ...
    rangePredicates.sort((a, b) => {
        if (a.lower === undefined)
            return -1;
        if (b.lower === undefined)
            return 1;
    ...
    for (const range of rangePredicates) {
        ...
        const canMerge = last.upper === undefined ||
 ...
 */
function getFieldType(columnName, fieldName, configs) {
 ...
 */
function evaluateFieldFilter(dataFile, fieldId, predicates, type, hasNeOperator, neValue) {
    // Get file bounds
    ...
    // Deserialize bounds
    const fileLower = lowerBytes instanceof Uint8Array
        ? deserializeShreddedBound(lowerBytes, type)
        : typeof lowerBytes === 'string'
            ? lowerBytes
            : undefined;
    const fileUpper = upperBytes instanceof Uint8Array
 ...
 * ```
 */
export function filterDataFiles(dataFiles, filter, configs, fieldIdMap) {
    ...
    return dataFiles.filter((dataFile) => {
        ...
        for (const ff of fieldFilters) {
            ...
            const matches = evaluateFieldFilter(dataFile, ff.fieldId, ff.predicates, ff.type, ff.hasUnknownOperators && ff.neValue !== undefined, ff.neValue);
            if (!matches) {
                return false;
            }
        }
 ...
 */
export function filterDataFilesWithStats(dataFiles, filter, configs, fieldIdMap) {
...
//# sourceMappingURL=row-group-filter.js.map