 ...
 */
export function getShredFieldsKey(columnName) {
 ...
 */
export function getFieldTypesKey(columnName) {
 ...
 */
export function parseShredColumnsProperty(value) {
    if (!value || value.trim() === '') {
        return [];
    }
    return value
 ...
 */
export function parseShredFieldsProperty(value) {
    if (!value || value.trim() === '') {
        return [];
    }
    return value
 ...
 */
export function parseFieldTypesProperty(value) {
    if (!value || value.trim() === '') {
        return {};
    }
    const result = {};
    ...
    for (const pair of pairs) {
        ...
        const typeName = trimmed.substring(colonIndex + 1).trim();
 ...
 * ```
 */
export function extractVariantShredConfig(properties) {
 ...
 */
export function formatShredColumnsProperty(configs) {
 ...
 */
export function formatShredFieldsProperty(fields) {
    return fields.join(',');
}
/**
 ...
 */
export function formatFieldTypesProperty(fieldTypes) {
 ...
 * ```
 */
export function toTableProperties(configs) {
    if (configs.length === 0) {
        return {};
    }
    const properties = {};
 ...
 * ```
 */
export function validateShredConfig(config) {
    const errors = [];
    ...
    for (const [fieldName, typeName] of Object.entries(config.fieldTypes)) {
        ...
        if (!VALID_PRIMITIVE_TYPES.has(typeName)) {
            errors.push(`Invalid field type '${typeName}' for field '${fieldName}'${config.columnName ? ` in column '${config.columnName}'` : ''}`);
        }
        // Check that field type references an existing field
        if (!declaredFields.has(fieldName)) {
            errors.push(`Field type declared for field '${fieldName}' which is not declared in fields array${config.columnName ? ` for column '${config.columnName}'` : ''}`);
        }
    }
    return {
