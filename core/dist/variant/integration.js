 ...
 */
import { extractVariantShredConfig, toTableProperties, } from './config.js';
 ...
 * ```
 */
export function parseShredConfig(properties) {
 ...
 * ```
 */
export function formatShredConfig(configs) {
    const internalConfigs = configs.map((config) => ({
 ...
 * ```
 */
export function createVariantSchemaFields(configs) {
 ...
 * ```
 */
export function getFieldIdForShreddedPath(fieldIdMap, shreddedPath) {
 ...
 * ```
 */
export function validateConfigWithSchema(configs, schema) {
    const errors = [];
 ...
 * ```
 */
export function setupVariantShredding(options) {
