 ...
 */
function getFieldIdForPath(statsPath, fieldIdMap) {
    return fieldIdMap.get(statsPath);
}
/**
 * Get bounds from a data file for a given field ID.
 */
function getBoundsFromDataFile(dataFile, fieldId, type) {
    ...
    // Deserialize bounds
    const lower = lowerBound instanceof Uint8Array
        ? deserializeShreddedBound(lowerBound, type)
        : lowerBound;
    const upper = upperBound instanceof Uint8Array
 ...
 */
export function boundsOverlapValue(lower, upper, operator, value, type) {
    switch (operator) {
        case '$eq': {
 ...
 */
export function evaluateInPredicate(lower, upper, values, type) {
 ...
 */
function evaluateFieldFilter(dataFile, fieldPath, filterValue, configs, fieldIdMap) {
 ...
 */
function evaluateOperatorFilter(dataFile, fieldId, fieldName, operator, value, type) {
 ...
 */
function evaluateNullFilter(dataFile, fieldId, fieldName, operator) {
 ...
 * Evaluate $ne filter.
 */
function evaluateNeFilter(dataFile, fieldId, fieldName, value, type) {
    // Handle $ne null
    if (value === null) {
 ...
 * Evaluate $in filter.
 */
function evaluateInFilter(dataFile, fieldId, fieldName, values, type) {
 ...
 */
function evaluateFilter(dataFile, filter, configs, fieldIdMap) {
 ...
 * ```
 */
export function shouldSkipDataFile(dataFile, filter, configs, fieldIdMap) {
    return evaluateFilter(dataFile, filter, configs, fieldIdMap);
}
//# sourceMappingURL=predicate-pushdown.js.map